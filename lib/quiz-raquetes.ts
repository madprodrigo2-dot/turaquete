// Generated by tools/quiz-raquetes.ts on 2026-08-08
// Do NOT edit manually. Re-run: npm run quiz:raquetes
// Single source of truth: same scorer + pipeline as Tury.
//
// 2026-09-11: patch manual pontual — slug/image_url/price de 10 entradas
// corrigidos contra o banco real (drift desde a geração). Não mudou quais
// raquetes aparecem nem a ordem por arquétipo. Próxima regeneração via
// npm run quiz:raquetes substitui este patch por completo.

import type { ArquetipoSlug } from './quiz-perfil'

export interface QuizRaqueteCard {
  slug:           string
  name:           string
  nome_curto:     string
  marca:          string
  price:          number | null
  image_url:      string | null
  destaques:      Array<{ label: string; v: number }>
  custoBeneficio: boolean
  score:          number
}

export const QUIZ_RAQUETES: Record<ArquetipoSlug, QuizRaqueteCard[]> = 
{
  "muralha": [
    {
      "slug": "vision-pyramid-25",
      "name": "Pyramid",
      "nome_curto": "Pyramid",
      "marca": "Vision",
      "price": 999,
      "image_url": "/raquetes/vision-pyramid-25.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 8
        },
        {
          "label": "Conforto",
          "v": 8
        }
      ],
      "custoBeneficio": false,
      "score": 7.1
    },
    {
      "slug": "ceu",
      "name": "CÉU",
      "nome_curto": "CÉU",
      "marca": "Heroe's",
      "price": 1424.05,
      "image_url": "/raquetes/ceu.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 8
        },
        {
          "label": "Controle",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 7
    },
    {
      "slug": "dropshot-renegade-comfort-bt",
      "name": "Renegade Comfort BT",
      "nome_curto": "Renegade Comfort BT",
      "marca": "Drop Shot",
      "price": 799.9,
      "image_url": "/raquetes/dropshot-renegade-comfort-bt.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 9
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.9
    },
    {
      "slug": "turquoise-tq-fire-azul",
      "name": "TQ Fire Azul",
      "nome_curto": "TQ Fire Azul",
      "marca": "Turquoise",
      "price": 2299,
      "image_url": "/raquetes/turquoise-tq-fire-azul.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 9
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.8
    },
    {
      "slug": "kona-maddox-guga",
      "name": "Maddox Guga",
      "nome_curto": "Maddox Guga",
      "marca": "Kona",
      "price": 913.26,
      "image_url": "/raquetes/kona-maddox-guga.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 9
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.7
    },
    {
      "slug": "mormaii-vitoria-marchezini-ii",
      "name": "Vitória Marchezini II",
      "nome_curto": "Vitória Marchezini II",
      "marca": "Mormaii",
      "price": 2037,
      "image_url": "/raquetes/mormaii-vitoria-marchezini-ii.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.7
    }
  ],
  "contra-atacante": [
    {
      "slug": "vision-pyramid-25",
      "name": "Pyramid",
      "nome_curto": "Pyramid",
      "marca": "Vision",
      "price": 999,
      "image_url": "/raquetes/vision-pyramid-25.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 8
        },
        {
          "label": "Conforto",
          "v": 8
        }
      ],
      "custoBeneficio": false,
      "score": 7.1
    },
    {
      "slug": "ceu",
      "name": "CÉU",
      "nome_curto": "CÉU",
      "marca": "Heroe's",
      "price": 1424.05,
      "image_url": "/raquetes/ceu.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 8
        },
        {
          "label": "Controle",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 7
    },
    {
      "slug": "dropshot-renegade-comfort-bt",
      "name": "Renegade Comfort BT",
      "nome_curto": "Renegade Comfort BT",
      "marca": "Drop Shot",
      "price": 799.9,
      "image_url": "/raquetes/dropshot-renegade-comfort-bt.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 9
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.9
    },
    {
      "slug": "turquoise-tq-fire-azul",
      "name": "TQ Fire Azul",
      "nome_curto": "TQ Fire Azul",
      "marca": "Turquoise",
      "price": 2299,
      "image_url": "/raquetes/turquoise-tq-fire-azul.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 9
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.8
    },
    {
      "slug": "kona-maddox-guga",
      "name": "Maddox Guga",
      "nome_curto": "Maddox Guga",
      "marca": "Kona",
      "price": 913.26,
      "image_url": "/raquetes/kona-maddox-guga.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 9
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.7
    },
    {
      "slug": "mormaii-vitoria-marchezini-ii",
      "name": "Vitória Marchezini II",
      "nome_curto": "Vitória Marchezini II",
      "marca": "Mormaii",
      "price": 2037,
      "image_url": "/raquetes/mormaii-vitoria-marchezini-ii.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.7
    }
  ],
  "canhao": [
    {
      "slug": "dropshot-canyon-pro-bt-10-2024",
      "name": "Canyon Pro BT 1.0",
      "nome_curto": "Canyon Pro BT 1.0",
      "marca": "Drop Shot",
      "price": 2540,
      "image_url": "/raquetes/dropshot-canyon-pro-bt-10-2024.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "poison-bee-2026",
      "name": "Poison Bee 2026",
      "nome_curto": "Poison Bee",
      "marca": "AMA Sport",
      "price": 2500,
      "image_url": "/raquetes/poison-bee-2026.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "kona-maverick-black-2025",
      "name": "Maverick Black",
      "nome_curto": "Maverick Black",
      "marca": "Kona",
      "price": 1329.05,
      "image_url": "/raquetes/kona-maverick-black-2025.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.5
    },
    {
      "slug": "zeiq-brave-rafa-miller-2025",
      "name": "Brave Rafa Miller 12k",
      "nome_curto": "Brave Rafa Miller 12k",
      "marca": "Zeiq",
      "price": 1917.13,
      "image_url": "/raquetes/zeiq-brave-rafa-miller-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.4
    },
    {
      "slug": "fobel-macaw-2025",
      "name": "Macaw 2025",
      "nome_curto": "Macaw",
      "marca": "Fobel",
      "price": 2399.9,
      "image_url": "/raquetes/fobel-macaw-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    },
    {
      "slug": "flash-2026",
      "name": "Flash 2026",
      "nome_curto": "Flash",
      "marca": "Minimalist",
      "price": 2279.05,
      "image_url": "/raquetes/minimalist-flash-2026.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    }
  ],
  "dono-da-rede": [
    {
      "slug": "dropshot-canyon-pro-bt-10-2024",
      "name": "Canyon Pro BT 1.0",
      "nome_curto": "Canyon Pro BT 1.0",
      "marca": "Drop Shot",
      "price": 2540,
      "image_url": "/raquetes/dropshot-canyon-pro-bt-10-2024.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "poison-bee-2026",
      "name": "Poison Bee 2026",
      "nome_curto": "Poison Bee",
      "marca": "AMA Sport",
      "price": 2500,
      "image_url": "/raquetes/poison-bee-2026.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "kona-maverick-black-2025",
      "name": "Maverick Black",
      "nome_curto": "Maverick Black",
      "marca": "Kona",
      "price": 1329.05,
      "image_url": "/raquetes/kona-maverick-black-2025.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.5
    },
    {
      "slug": "zeiq-brave-rafa-miller-2025",
      "name": "Brave Rafa Miller 12k",
      "nome_curto": "Brave Rafa Miller 12k",
      "marca": "Zeiq",
      "price": 1917.13,
      "image_url": "/raquetes/zeiq-brave-rafa-miller-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.4
    },
    {
      "slug": "fobel-macaw-2025",
      "name": "Macaw 2025",
      "nome_curto": "Macaw",
      "marca": "Fobel",
      "price": 2399.9,
      "image_url": "/raquetes/fobel-macaw-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    },
    {
      "slug": "flash-2026",
      "name": "Flash 2026",
      "nome_curto": "Flash",
      "marca": "Minimalist",
      "price": 2279.05,
      "image_url": "/raquetes/minimalist-flash-2026.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    }
  ],
  "finalizador": [
    {
      "slug": "dropshot-canyon-pro-bt-10-2024",
      "name": "Canyon Pro BT 1.0",
      "nome_curto": "Canyon Pro BT 1.0",
      "marca": "Drop Shot",
      "price": 2540,
      "image_url": "/raquetes/dropshot-canyon-pro-bt-10-2024.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "poison-bee-2026",
      "name": "Poison Bee 2026",
      "nome_curto": "Poison Bee",
      "marca": "AMA Sport",
      "price": 2500,
      "image_url": "/raquetes/poison-bee-2026.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "kona-maverick-black-2025",
      "name": "Maverick Black",
      "nome_curto": "Maverick Black",
      "marca": "Kona",
      "price": 1329.05,
      "image_url": "/raquetes/kona-maverick-black-2025.webp",
      "destaques": [
        {
          "label": "Potência",
          "v": 7
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.5
    },
    {
      "slug": "zeiq-brave-rafa-miller-2025",
      "name": "Brave Rafa Miller 12k",
      "nome_curto": "Brave Rafa Miller 12k",
      "marca": "Zeiq",
      "price": 1917.13,
      "image_url": "/raquetes/zeiq-brave-rafa-miller-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.4
    },
    {
      "slug": "fobel-macaw-2025",
      "name": "Macaw 2025",
      "nome_curto": "Macaw",
      "marca": "Fobel",
      "price": 2399.9,
      "image_url": "/raquetes/fobel-macaw-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    },
    {
      "slug": "flash-2026",
      "name": "Flash 2026",
      "nome_curto": "Flash",
      "marca": "Minimalist",
      "price": 2279.05,
      "image_url": "/raquetes/minimalist-flash-2026.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.4
    }
  ],
  "camaleao": [
    {
      "slug": "ceu",
      "name": "CÉU",
      "nome_curto": "CÉU",
      "marca": "Heroe's",
      "price": 1424.05,
      "image_url": "/raquetes/ceu.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 8
        },
        {
          "label": "Controle",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.8
    },
    {
      "slug": "vision-pyramid-25",
      "name": "Pyramid",
      "nome_curto": "Pyramid",
      "marca": "Vision",
      "price": 999,
      "image_url": "/raquetes/vision-pyramid-25.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 8
        },
        {
          "label": "Conforto",
          "v": 8
        }
      ],
      "custoBeneficio": false,
      "score": 6.8
    },
    {
      "slug": "athena-boreal-26",
      "name": "Athena Boreal",
      "nome_curto": "Athena Boreal",
      "marca": "AMA Sport",
      "price": 2885.67,
      "image_url": "/raquetes/athena-boreal-26.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 8
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "dropshot-renegade-comfort-bt",
      "name": "Renegade Comfort BT",
      "nome_curto": "Renegade Comfort BT",
      "marca": "Drop Shot",
      "price": 799.9,
      "image_url": "/raquetes/dropshot-renegade-comfort-bt.webp",
      "destaques": [
        {
          "label": "Conforto",
          "v": 9
        },
        {
          "label": "Manuseio",
          "v": 7
        }
      ],
      "custoBeneficio": true,
      "score": 6.6
    },
    {
      "slug": "mormaii-vitoria-marchezini-ii",
      "name": "Vitória Marchezini II",
      "nome_curto": "Vitória Marchezini II",
      "marca": "Mormaii",
      "price": 2037,
      "image_url": "/raquetes/mormaii-vitoria-marchezini-ii.webp",
      "destaques": [
        {
          "label": "Controle",
          "v": 7
        },
        {
          "label": "Conforto",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.6
    },
    {
      "slug": "zeiq-brave-rafa-miller-2025",
      "name": "Brave Rafa Miller 12k",
      "nome_curto": "Brave Rafa Miller 12k",
      "marca": "Zeiq",
      "price": 1917.13,
      "image_url": "/raquetes/zeiq-brave-rafa-miller-2025.webp",
      "destaques": [
        {
          "label": "Manuseio",
          "v": 7
        },
        {
          "label": "Spin",
          "v": 7
        }
      ],
      "custoBeneficio": false,
      "score": 6.5
    }
  ]
}
