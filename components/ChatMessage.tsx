  ...
  return (
    <div className={`flex flex-col w-full msg-enter ${isAssistant ? 'items-start' : 'items-end'}`}>
        ...
        {(loading || isPacing || content) && (
          <div
            className={`px-4 py-3 rounded-2xl text-sm md:text-[15px] leading-relaxed
              ${isAssistant
                ? `${turyConfig ? 'max-w-[85%]' : 'w-full'} bg-white text-tinta/85 rounded-tl-sm shadow-sm border border-tinta/[0.07]`
                : 'max-w-[80%] break-words ml-auto bg-tinta text-white rounded-tr-sm'
              }`}
      ...
      {isAssistant && suggestions && suggestions.length > 0 && onSuggestion && !holdBack && (
        <div className="mt-3 pl-[46px] md:pl-[68px] w-full flex flex-wrap gap-1.5">
          {suggestions.map(s => (
            <button
              key={s}
              onClick={() => onSuggestion(s)}
              className="text-xs md:text-sm font-medium px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl border border-aqua/40 text-tinta/70 bg-white hover:bg-aqua/10 hover:border-aqua/60 active:scale-[0.97] transition-all shadow-sm"
            >
              {s}
            </button>
          ))}
