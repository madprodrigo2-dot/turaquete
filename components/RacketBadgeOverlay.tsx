...
export default function RacketBadgeOverlay({ athlete, brandLogo, brandName }: Props) {
  ...
  // Also match the brand badge's box height (h-6/md:h-9) so both sit on the
  // same horizontal line — min-height (not height) so a wrapped multi-athlete
  // pill can still grow past it instead of getting clipped.
  ...
  return (
    <>
      ...
      {brandLogo && (
        <div className="absolute top-1.5 right-1.5 z-10 max-w-[calc(100%-12px)] h-6 md:h-9 flex items-center justify-center rounded-md bg-white/95 border border-tinta/10 shadow-sm px-1.5 md:px-2">
